...
class MedicalDataPreprocessor:
    ...
    def scale_features(self, features, is_train=True):
        ...
        print(f"✓ Features scaled: {len(numerical_cols)} numerical columns")
        return features
    ...
    def remove_correlated_features(self, features, correlation_threshold=0.95):
        ...
        Args:
            features (pd.DataFrame): Feature dataframe
            correlation_threshold (float): Remove if |correlation| > threshold
            
        Returns:
            pd.DataFrame: Features with correlated ones removed
        """
    ...
    def select_features(self, features, targets_primary, method='combined'):
        ...
        Args:
            features (pd.DataFrame): Feature dataframe
            targets_primary (pd.Series): Primary target for feature selection (diabetes)
            method (str): 'mutual_info', 'rfe', or 'combined'
            
        Returns:
            tuple: (selected_features_df, selected_feature_names)
        """
        ...
        print(f"✓ Features selected: {len(selected)}/{n_original}")
        print(f"  Selected features: {selected[:10]}{'...' if len(selected) > 10 else ''}")
    ...
    def handle_class_imbalance(self, X, y_target, method='smote'):
        ...
        if method == 'smote':
            ...
            try:
                ...
                print(f"✓ SMOTE applied: {original_balance} → {new_balance}")
                return X_balanced, y_balanced
            except Exception as e:
                print(f"⚠ SMOTE failed: {e}. Proceeding without resampling.")
                return X, y_target
    ...
    def fit(self, data_path):
        ...
        Returns:
            dict: Preprocessed training data and relevant information
        """
        print("\n" + "="*60)
    ...
    def transform(self, features):
        ...
        Args:
            features (pd.DataFrame): Raw feature dataframe
