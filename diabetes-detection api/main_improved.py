...
2. Multiple model training with cross-validation
...
from data_preprocessing_improved import MedicalDataPreprocessor
...
class EnhancedPipelineOrchestrator:
    ...
    def __init__(self, data_path='data/clinical_data.csv', random_state=42, cv=5):
        ...
        Args:
            data_path (str): Path to clinical data CSV
    ...
    def run_complete_pipeline(self):
        ...
        print("\n" + "="*60)
        print("=" + " "*58 + "=")
        print("=" + " ENHANCED MULTI-DISEASE PREDICTION PIPELINE ".center(58) + "=")
        print("=" + " "*58 + "=")
        print("="*60)
        ...
        # Step 3: Hyperparameter Tuning (optional - can take time)
        self.step_hyperparameter_tuning()
    ...
    def step_hyperparameter_tuning(self):
        """Execute hyperparameter tuning (optional)."""
        print("\n" + "-"*60)
        print("STEP 3: HYPERPARAMETER OPTIMIZATION (Optional)")
        print("-"*60)
        print("\n⚠ Note: Hyperparameter tuning can take 10-30 minutes.")
        print("  Uncomment the code below in the main script to enable.")
        print("  Currently using default parameters for faster execution.\n")
        
        # Uncomment to enable hyperparameter tuning
        # optimizer = HyperparameterOptimizer(cv=self.cv, random_state=self.random_state)
        # optimizer.optimize_all(self.X_train, self.y_diabetes, search_type='random', n_iter=20)
    ...
    def step_generate_plots(self):
        ...
        print("✓ Saved: results/cv_analysis_diabetes.png")
        plt.close(fig4)
